...
# 生成蓝图对象,管理功能
from back.models import User, Article, ArcticleType, db
...
@back_blue.route('/rigister/',methods=['GET','POST'])
def rigister():
...
def del_type(id):
    atype = ArcticleType.query.get(id)
...
def article_list():
    acticles = Article.query.all()
    return  render_template('back/article_list.html',acticles=acticles)
